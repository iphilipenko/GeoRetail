-- =====================================================
-- Прив'язка H3-8 гексагонів до адміністративних одиниць
-- Файл: sql/15_populate_h3_8_mapping.sql
-- =====================================================
...
VALUES ('h3_8_mapping', 'started', 'Початок прив''язки H3-8 гексагонів (759,291 штук)');
...
CREATE INDEX IF NOT EXISTS idx_temp_h3_8_oblast ON temp_h3_8_batch(oblast_id);
CREATE INDEX IF NOT EXISTS idx_temp_h3_8_center ON temp_h3_8_batch USING GIST(center_point);
...
DROP PROCEDURE IF EXISTS process_h3_8_mapping();

-- 6. Створюємо процедуру для батчевої обробки з COMMIT
CREATE PROCEDURE process_h3_8_mapping()
LANGUAGE plpgsql
AS $$
DECLARE
    oblast_rec RECORD;
...
BEGIN
    ...
    LOOP
        ...
        -- Територіальна громада (рівень 7) - тільки якщо в районі
        ...
        -- Логуємо прогрес кожні 3 області
...
SELECT 
    ...
    ROUND(100.0 * with_settlement / NULLIF(total_mapped, 0), 2) as "% з населеним пунктом",
    in_communities as "В громадах",
    in_settlements_8 as "В нас. пунктах 8",
    in_settlements_9 as "В нас. пунктах 9"
FROM stats;

-- 10. Перевірка покриття
SELECT 
    'Покриття H3-8' as check_type,
    (SELECT COUNT(*) FROM osm_ukraine.h3_grid WHERE resolution = 8) as total_h3_8,
    (SELECT COUNT(*) FROM osm_ukraine.h3_admin_mapping WHERE h3_resolution = 8) as mapped_h3_8,
    (SELECT COUNT(*) FROM osm_ukraine.h3_grid WHERE resolution = 8) - 
    (SELECT COUNT(*) FROM osm_ukraine.h3_admin_mapping WHERE h3_resolution = 8) as not_mapped;